...
export default function FieldDashboard() {
  ...
  // FORM STATE: remarks is now an array of 5 strings
  const [formData, setFormData] = useState({
    className: '',
    directorName: '',
    directorNumber: '',
    ...
    classCount: '',
    subjectsTaught: '',
    remarks: ['', '', '', '', ''] // 5 empty strings
  });

  const [userLocation, setUserLocation] = useState({ latitude: 19.076, longitude: 72.8777 });

  const updateRemark = (text: string, index: number) => {
  ...
  useEffect(() => {
    ...
    let dist = 0;
    for (let i = 0; i < pathCoordinates.length - 1; i++) {
      dist += calculateDistance(pathCoordinates[i].latitude, pathCoordinates[i + 1].latitude, pathCoordinates[i].longitude, pathCoordinates[i + 1].longitude);
    }
    setTotalDistance(dist);
  }, [pathCoordinates]);
  ...
  const saveEntryToLog = async () => {
    ...
    try {
      ...
      setFormData({
        className: '', directorName: '', directorNumber: '', address: '',
        contactPersonName: '', contactPersonNumber: '', studentCount: '',
        classCount: '', subjectsTaught: '', remarks: ['', '', '', '', '']
      });
      setNoteModalVisible(false);
      Alert.alert("Success", "Entry saved.");
    } 
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.modalOverlay}>
          <View style={styles.formCard}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
              ...
              <Text style={styles.inputLabel}>What they teach</Text>
              <TextInput style={styles.formInput} placeholder="JEE, NEET, Boards..." value={formData.subjectsTaught} onChangeText={(t) => setFormData({ ...formData, subjectsTaught: t })} />
              ...
              {/* Remarks Section: 5 Dynamic Text Fields */}
              <Text style={[styles.inputLabel, {marginTop: 20}]}>Remarks / Notes (5 items)</Text>
              {formData.remarks.map((remark, index) => (
                <TextInput 
                  ...
                  value={remark} 
                  onChangeText={(t) => updateRemark(t, index)} 
                />
              ))}
